<x-app>
    ...
    <div class="card shadow-lg p-3">
        <form action="{{ route('feedbacks.update', $feedback) }}" method="post" class="form">
            @csrf
            @method('put')
                ...
                <div class="col-md-8 mx-auto">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('rating') is-invalid @enderror" id="rating" name="rating" required>
                            <option value="5" {{ old('rating', $feedback->rating) == 5 ? 'selected' : '' }}>5 - Sangat Memuaskan</option>
                            <option value="4" {{ old('rating', $feedback->rating) == 4 ? 'selected' : '' }}>4 - Memuaskan</option>
                            ...
                            <option value="1" {{ old('rating', $feedback->rating) == 1 ? 'selected' : '' }}>1 - Sangat Kurang</option>
                        </select>
                        @error('rating')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('comments') is-invalid @enderror" id="comments"
                            name="comments" rows="4">{{ old('comments', $feedback->comments) }}</textarea>
                        @error('comments')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
